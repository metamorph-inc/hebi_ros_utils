...
class HebirosWrapper(object):
    def __init__(self, hebi_group_name, hebi_families=None, hebi_names=None, from_urdf=False, lite_mode=False):
        ...
        if not from_urdf:
            ...
            rospy.loginfo("  Waiting for AddGroupFromNamesSrv at %s%s ...", rospy.get_namespace(), 'hebiros/add_group_from_names')
            ...
            rospy.loginfo("  Waiting for AddGroupFromURDFSrv at %s%s ...", rospy.get_namespace(), 'hebiros/add_group_from_urdf')
        ...
        rospy.loginfo("  feedback_topic: %s%s", rospy.get_namespace(), self.feedback_topic)
        self.feedback_joint_state_topic = 'hebiros/'+hebi_group_name+'/feedback/joint_state'
        rospy.loginfo("  feedback_joint_state_topic: %s%s", rospy.get_namespace(), self.feedback_joint_state_topic)
        self.command_topic = 'hebiros/'+hebi_group_name+'/command'
        rospy.loginfo("  command_topic: %s%s", rospy.get_namespace(), self.command_topic)
        self.command_joint_state_topic = 'hebiros/'+hebi_group_name+'/command/joint_state'
        rospy.loginfo("  command_joint_state_topic: %s%s", rospy.get_namespace(), self.command_joint_state_topic)

        # Services
        self.entry_list_srv = 'hebiros/entry_list'
        rospy.loginfo("  entry_list_srv: %s%s", rospy.get_namespace(), self.entry_list_srv)
        self.send_command_with_acknowledgement_srv = 'hebiros/'+hebi_group_name+'/send_command_with_acknowledgement'
        rospy.loginfo("  send_command_with_acknowledgement_srv: %s%s", rospy.get_namespace(), self.send_command_with_acknowledgement_srv)
        self.set_command_lifetime_srv = 'hebiros/'+hebi_group_name+'/set_command_lifetime'
        rospy.loginfo("  set_command_lifetime_srv: %s%s", rospy.get_namespace(), self.set_command_lifetime_srv)
        self.set_feedback_frequency_srv = 'hebiros/'+hebi_group_name+'/set_feedback_frequency'
        rospy.loginfo("  set_feedback_frequency_srv: %s%s", rospy.get_namespace(), self.set_feedback_frequency_srv)
        self.size_srv = 'hebiros/'+hebi_group_name+'/size'
        rospy.loginfo("  size_srv: %s%s", rospy.get_namespace(), self.size_srv)

        # Actions
        self.trajectory_action = 'hebiros/'+hebi_group_name+'/trajectory'
        rospy.loginfo("  trajectory_action: %s%s", rospy.get_namespace(), self.trajectory_action)
